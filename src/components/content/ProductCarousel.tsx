...
const ProductCarousel = ({ section, showHeader = true }: Props) => {
  const { data: dbProducts, isLoading } = useProducts();
  const titleRef = useRef<HTMLDivElement>(null);
  const isTitleInView = useInView(titleRef, { once: true, margin: "-50px" });

  const sectionTitle = section?.title || "Destaques";
  const sectionSubtitle = section?.subtitle || "Seleção";
  ...
  const products = dbHasImages
    ...
    : fallbackProducts.map((p) => ({ ...p, hoverImage: "", isNew: p.id === "3" }));
    ...
    <section data-theme-section="product-carousel" className={sectionClassName}>
      {showHeader && (
        <motion.div ref={titleRef} initial={{ opacity: 0, y: 30 }} animate={isTitleInView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.6 }} className="flex justify-between items-end mb-10 md:mb-14">
          <div>
                      ...
                      <div className="aspect-[3/4] mb-4 overflow-hidden relative bg-muted/20">
                        ...
                        {product.hoverImage && (
                          <img src={product.hoverImage} alt={`${product.name} hover`}
                            className="absolute inset-0 w-full h-full object-cover transition-all duration-700 opacity-0 group-hover:opacity-100" loading="lazy" />
                        )}
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 transition-colors duration-500" />
                        {product.isNew && (
                          <div className="absolute top-3 left-3">
                            <span className="text-editorial text-[10px] tracking-[0.15em] bg-foreground text-background px-3 py-1">NOVO</span>
                          </div>
                        )}
                      </div>
                      <div className="space-y-1">
                        <p className="text-editorial text-[10px] text-muted-foreground tracking-[0.12em]">{product.category}</p>
                        <div className="flex justify-between items-center gap-4">
                          <h3 className="text-sm font-medium text-foreground group-hover:opacity-70 transition-opacity duration-300">{product.name}</h3>
                          <p className="text-sm font-light text-foreground whitespace-nowrap">R${product.price.toLocaleString("pt-BR")}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
      ...
      {showHeader && (
        <div className="mt-8 md:hidden">
          <Link to={linkUrl} className="inline-flex items-center gap-2 text-foreground text-sm font-light border-b border-foreground/30 pb-1">
            <span>{ctaText}</span>
